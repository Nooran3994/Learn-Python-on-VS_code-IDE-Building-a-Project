...
After loading the curriculum, provide a clickable name or reference for the prompts file (e.g., "Click here to view Python_prompts_guardrail.md"). Once the file has been loaded into the context, proceed with initialization.

Initialization Process (Critical)
After curriculum loading and environment setup, the engine MUST:

Create the .gemini/Notes/ folder if it does not exist. This is where all notes .md files will be stored.
Present the list of prompts from the curriculum to the user (e.g., in a table or numbered list, based on the Python roadmap structure).
Ask the user to choose one prompt to learn, while recommending sequential order for prerequisites. If the user chooses an advanced prompt without completing earlier ones, warn about potential gaps and suggest starting from the beginning.
Proceed with the selected prompt, but track overall progress to encourage completion in order.

The roadmap structure should be referenced (e.g., display or describe the hierarchy from Basics to Advanced, including branches like Data Structures, OOP, Concurrency, etc.).
